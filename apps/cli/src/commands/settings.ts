...
// Settings page HTML
// ---------------------------------------------------------------------------
function settingsPage(store: ReturnType<typeof loadStore>): string {
...
export async function settingsCommand(): Promise<void> {
  ...
  const settingsUrl = `http://127.0.0.1:${port}`
  ...
  await new Promise<void>((resolve) => {
    server.on('request', (req, res) => {
      ...
      if (req.method === 'GET' && url.pathname === '/') {
        ...
        res.writeHead(200, { 'Content-Type': 'text/html' }).end(settingsPage(store))
        return
      }
      ...
      if (req.method === 'POST' && url.pathname === '/api/active') {
        ...
        req.on('end', () => {
          try {
            ...
            if (!apiUrl) { res.writeHead(400).end('{}'); return }
